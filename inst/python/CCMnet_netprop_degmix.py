...
def calc_f_degmix(g_net_stat, proposal_edge, g_proposal_edge, covPattern, bayesian_inference, P_net_stat, g, f_g_g2_bool, print_calculations):
	...
	if not is_removing:
		# ADDITION
		if d_u <= 0 or d_v <= 0: return 1.0
		if d_u == d_v:
			term = deg_dist[d_u] * (deg_dist[d_u] - 1) * 0.5
		else:
			term = deg_dist[d_u] * deg_dist[d_v]
		log_prob = np.log(max(1e-15, term - g_net_stat[idx_u, idx_v]))

		if d_u == d_v:
			log_denom = log_comb(deg_dist[d_u] * d_u, d_u + d_v)
			log_num = sum(log_comb(working_dmm[idx_u, k], n_i[k] + n_j[k]) for k in range(num_deg_stats))
			log_prob += safe_diff(log_num, log_denom)
		else:
			log_num_u = sum(log_comb(working_dmm[idx_u, k], n_i[k]) for k in range(num_deg_stats))
			log_num_v = sum(log_comb(working_dmm[idx_v, k], n_j[k]) for k in range(num_deg_stats))
