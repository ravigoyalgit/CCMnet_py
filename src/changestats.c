...
//Rprintf("NODEMIX: Node 1 %f Node 2 %f\n",rtype, ctype);    
...
D_CHANGESTAT_FN(d_degmix){
  ...
  int i, j, echange, changes, maxdeg, ninputs, ninputs2;
  Edge e;
//  double changesum;
//  int k;
  
  ninputs = N_INPUT_PARAMS - 1;
  ninputs2 = ninputs/2;
  maxdeg = INPUT_PARAM[N_INPUT_PARAMS-1];

  ZERO_ALL_CHANGESTATS(i);
  FOR_EACH_TOGGLE(i) {
    tail = TAIL(i); head = HEAD(i);
    
    /*Find the degrees for the tail and head*/
    taildeg = OUT_DEG[tail] + IN_DEG[tail];
    headdeg = OUT_DEG[head] + IN_DEG[head];
    if(taildeg > maxdeg) taildeg = maxdeg;
    if(headdeg > maxdeg) headdeg = maxdeg;
    echange = IS_OUTEDGE(tail, head) ? -1 : 1;

//Rprintf("tail %d, head %d \n", taildeg, headdeg);
//Rprintf("change %d \n", echange);
  
    if(taildeg == 0 && headdeg ==0){
      CHANGE_STAT[0] += 1;
      //changesum = 0;
      //for(k=0; k<ninputs2; k++){
      //  changesum += CHANGE_STAT[k];
      //}
      //Rprintf("changesum = %f\n", changesum);
    }else if(headdeg==0){
      for(j=0; j<ninputs2; j++){
        if((INPUT_PARAM[j] == 1) && (INPUT_PARAM[j+ninputs2] == MIN(taildeg+echange, maxdeg))){
          CHANGE_STAT[j] += 1;
          j = ninputs2; /* leave the for loop */
        }
      }
      ...
      STEP_THROUGH_OUTEDGES(tail, e, node3){
        node3deg = OUT_DEG[node3] + IN_DEG[node3];
        if(node3deg > maxdeg) node3deg = maxdeg;
        /*Find the right statistic to update */
        changes = 0;
        for(j=0; j<ninputs2; j++){
          if((INPUT_PARAM[j] == node3deg) && (INPUT_PARAM[j+ninputs2] == taildeg)){
            CHANGE_STAT[j] += -1;
            changes++;
          }
          if((INPUT_PARAM[j] == node3deg) && (INPUT_PARAM[j+ninputs2] == MIN(taildeg+echange, maxdeg))){
            CHANGE_STAT[j] += 1;
            changes++;
          }
          if((INPUT_PARAM[j] == MIN(taildeg+echange, maxdeg)) && (INPUT_PARAM[j+ninputs2] == node3deg) && (node3deg != MIN(taildeg+echange, maxdeg))){
            CHANGE_STAT[j] += 1;
            changes++;
          }
          if((INPUT_PARAM[j] == taildeg) && (INPUT_PARAM[j+ninputs2] == node3deg) && node3deg != taildeg){
            CHANGE_STAT[j] += -1;
            changes++;
          }
          if(changes == 2)  j = ninputs2; /* leave the for loop */
        } 
      }
      ...
      for(j=0; j<ninputs2; j++){
        if((INPUT_PARAM[j] == 1) && (INPUT_PARAM[j+ninputs2] == MIN(headdeg+echange, maxdeg))){
          CHANGE_STAT[j] += 1;
          j = ninputs2; /* leave the for loop */
        }
      }
      ...
      STEP_THROUGH_OUTEDGES(head, e, node3){
        ...
        changes = 0;
        for(j=0; j<ninputs2; j++){
          if((INPUT_PARAM[j] == node3deg) && (INPUT_PARAM[j+ninputs2] == headdeg)){
            CHANGE_STAT[j] += -1;
            changes++;
          }
          if((INPUT_PARAM[j] == node3deg) && (INPUT_PARAM[j+ninputs2] == MIN(headdeg+echange, maxdeg))){
            CHANGE_STAT[j] += 1;
            changes++;
          }
          if((INPUT_PARAM[j] == headdeg) && (INPUT_PARAM[j+ninputs2] == node3deg) && (node3deg != headdeg)){
            CHANGE_STAT[j] += -1;
            changes++;
          }
          if((INPUT_PARAM[j] == MIN(headdeg+echange, maxdeg)) && (INPUT_PARAM[j+ninputs2] == node3deg) && (MIN(headdeg + echange, maxdeg) != node3deg)){
            CHANGE_STAT[j] += 1;
            changes++;
          }
          if(changes == 2)  j = ninputs2; /* leave the for loop */
      ...
      STEP_THROUGH_INEDGES(head, e, node3){
        ...
        for(j=0; j<ninputs2; j++){
          ...
          if((INPUT_PARAM[j] == MIN(headdeg+echange, maxdeg)) && (INPUT_PARAM[j+ninputs2] == node3deg) && (node3deg != MIN(headdeg + echange, maxdeg))){
            CHANGE_STAT[j] += 1;
            changes++;
          }
          if(changes == 2)  j = ninputs2; /* leave the for loop */
      ...
      if(echange == 1){
        for(j=0; j<ninputs2; j++){
          if((INPUT_PARAM[j] == MIN(taildeg+echange, maxdeg)) && (INPUT_PARAM[j+ninputs2] == MIN(headdeg+echange, maxdeg))){
            CHANGE_STAT[j] += 1;
            j = ninputs2;
          }
          if((INPUT_PARAM[j] == MIN(headdeg+echange, maxdeg)) && (INPUT_PARAM[j+ninputs2] == MIN(taildeg+echange, maxdeg)) && (headdeg != taildeg)){
            CHANGE_STAT[j] += 1;
            j = ninputs2;
          }
        }
      ...
      if(echange == -1){
        for(j=0; j<ninputs2; j++){
          if((INPUT_PARAM[j] == headdeg) && (INPUT_PARAM[j+ninputs2] == taildeg)){
            CHANGE_STAT[j] += -1;
            j = ninputs2;
          }
          if((INPUT_PARAM[j] == taildeg) && (INPUT_PARAM[j+ninputs2] == headdeg) && (headdeg != taildeg)){
            CHANGE_STAT[j] += -1;
            j = ninputs2;
          }
        }
  ...
  /*Find the right statistic to update */
    ...
    STEP_THROUGH_OUTEDGES(tail, e, node3){
      ...
      for(j=0; j<ninputs2; j++){
        ...
        if((INPUT_PARAM[j] == taildeg+echange) && (INPUT_PARAM[j+ninputs2] == node3deg) && (MIN(taildeg + echange, maxdeg) != node3deg)){
          CHANGE_STAT[j] += 1;
          changes++;
        }
        if(changes == 2)  j = ninputs2; /* leave the for loop */
