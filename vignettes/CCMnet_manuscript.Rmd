...
##Analysis Workflow
Using CCMnet for network analysis typically follows a three-step process:

> **Model Specification**: Defining the network properties of interest and their corresponding probability distributions.

> **MCMC Sampling**: Using the package's computational backend to generate a representative ensemble of networks.

> **Verification and Diagnostics**: Assessing the sampler's performance and validating that the generated networks align with the specified theoretical targets.
...
for (num_sample_nodes in seq(0,240,40)) {
  ...
  n_samples <- nrow(ccm_sample$mcmc_stats)
